...
def assemble(stage: int, brief: Path, instruction: Path, out: Path,
    ...
    write_exclusive(out, payload.encode("utf-8"))
    ...
    write_exclusive(out.parent / "payload-manifest.json", (json.dumps(manifest, ensure_ascii=False, indent=2)+"\n").encode("utf-8"))
    return manifest
...
def build_retry_payload(original_payload: Path, diagnostics: list[str], out: Path) -> dict:
    ...
    if "DETERMINISTIC GATE DIAGNOSTICS â€” RETRY" in original or "DETERMINISTIC GATE DIAGNOSTICS  -  RETRY" in original:
        raise ValueError("retry source must be the frozen original stage payload; recursive retry chaining is prohibited")
    ...
    write_exclusive(out.parent / "retry-payload-manifest.json", (json.dumps(result, ensure_ascii=False, indent=2)+"\n").encode("utf-8"))
    return result
...
def gate_stage3(data, trace, material):
    ...
    if not isinstance(data.get("mode"), str) or data["mode"].lower() not in {"ionian","dorian","phrygian","lydian","mixolydian","aeolian","locrian"}: errors.append("invalid enum: $.mode")
        ...
        for i,t in enumerate(tracks):
            ...
            if isinstance(assignments,list):
                for j,a in enumerate(assignments):
                    if not isinstance(a,dict) or "section_id" not in a: errors.append(f"missing required field: $.tracks[{i}].section_assignments[{j}].section_id")
                    elif not ("motif_id" in a or "variation" in a): errors.append(f"missing required field: $.tracks[{i}].section_assignments[{j}].motif_id or variation")
                    else:
                        if isinstance(sections,list) and a["section_id"] not in ids: errors.append(f"invalid section reference: $.tracks[{i}].section_assignments[{j}].section_id")
                        if "motif_id" in a and isinstance(motifs,list) and a["motif_id"] not in motif_ids: errors.append(f"invalid motif reference: $.tracks[{i}].section_assignments[{j}].motif_id")
